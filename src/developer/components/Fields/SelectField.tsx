...
export default function SelectField(props: ISelectField) {
    ...
    return (
        <>
            <Listbox {...props} value={selected} onChange={(e) => { props.onChange(e.id); setSelected(e) }} {...(props.allowMulitple && { multile: true })}>
                {({ open }) => (
                    <>
                        <div className="mt-1 relative">
                            <BaseLabel {...props} />
                            <Listbox.Button className="h-10 mt-1 bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-ring-blue-500 focus:border-ring-blue-500 sm:text-sm">
